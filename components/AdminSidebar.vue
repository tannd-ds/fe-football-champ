  ...
  <AppCard class="h-[98vh] min-w-[312px] ml-3">
    ...
    <div class="mt-8 flex flex-col gap-2">
      <span 
        ...
        class="px-4 py-2 flex gap-4 items-center rounded-md hover:bg-zinc-800 cursor-pointer transition-all duration-200 ease-in-out"
        @click="router.push(link.path)"
      >
        <UIcon :name="link.icon" class="text-2xl" />
        <ULink :to="link.path" class="text-lg">{{ link.name }}</ULink>
      </span>
    </div>
  </AppCard>
</template>
